...
const isBunRuntime = typeof bunRuntime !== 'undefined';
        ...
        .then((inUse) => {
          ...
          try {
            if (isBunRuntime && bunRuntime) {
              const serverInstance = bunRuntime.serve({
                fetch: app.fetch,
                port,
                hostname: host,
              });
              const serverAddress = `http://${host}:${port}${config.mcpHttpEndpointPath}`;
            ...
            void import('@hono/node-server').then(({ serve }) => {
              ...
              resolve(serverInstance as ServerLike);
            });
          } catch (err: unknown) {
            logger.warning(
...
export async function stopHttpTransport(
  ...
  if (server.stop) {
    server.stop(true);
    logger.info('HTTP server stopped successfully.', operationContext);
    return;
  }
}
